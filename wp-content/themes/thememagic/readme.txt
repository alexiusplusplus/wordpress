=== thememagic ===
Author: Manishg
Tags: black,two-columns,theme-options
Requires at least: WP 3.0

== Description ==

A very neat and clean black and white business theme. The theme supports widgets. And features theme-options, threaded-comments and multi-level dropdown menu. A simple and neat typography.

== Support ==
Support forum at http://themeszen.com/forums-support

== Installation ==

You can install the theme through the WordPress installer under Themes-Install themes by searching for it.
Alternatively you can download the file from here, unzip it and move the unzipped contents to the wp-content/themes folder
of your WordPress installation. You will then be able to activate the theme.

== Frequently Asked Questions ==

= How to setup Blog page =
Create a new page by going to pages - Add New. Then on right side, in page attributes - Template, select Blog as the page template. And publish the page.

= Where can I get support for the theme? =
Support is available at the http://themeszen.com/forums-support.

= How to use menus =
For Menus: Use wordpress Menu system under Appearance tab. Also, make sure menu titles and child links are not long as they might overlap.

= Note for logo height =
Make sure your site title is not long enough and if you are using logo as image, make sure the image height is not more than 110px.


== Credits ==
jQuery http://jquery.com/

Thanks for downloading the thememagic theme.

