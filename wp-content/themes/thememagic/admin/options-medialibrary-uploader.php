...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) {
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels' => array(
				'name' => __( 'Options Framework Internal Container' , 'thememagic'),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload' , 'thememagic' ) . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$output .= '<div class="no_image"><span class="file_link"><a href="' . $value . '" thememagic="_blank" rel="external">'.$title.'</a></span>' . $remove . '</div>';
			}	
		}
		$output .= '</div>' . "\n";
		return $output;
